...
Статический инструмент для настройки пунктов меню: **права (`meta.roles`)** и **`meta.accessInfoKeys`** (флаги доступа из `accessInfoKeys.json`).
...
3. Выберите узел в дереве и отметьте права и флаги.
4. **Экспорт результата** сохраняет `routes.mapped.json` с обновлёнными `meta.roles` и `meta.accessInfoKeys`.

## Дополнительный импорт

- **Импорт результата** — дерево маршрутов с теми же правами и флагами (по позиции узла в дереве).
- **Импорт roles.json / routes.json / accessInfoKeys.json** — опционально, подмена исходников без пересборки `data.js`.

## Правила по умолчанию

- Если у маршрута **`meta.roles`** нет или пусто — считается, что **не выбрано ни одно право**.
- Если **`meta.accessInfoKeys`** нет или пусто — считается, что **не выбран ни один флаг**.
- У **родительских** узлов (есть `children`) права и флаги **не редактируются вручную**: показывается объединение по потомкам, в экспорт уходит то же объединение.
...
Если хочешь, могу сразу подготовить `/.github/workflows/deploy.yml` для автодеплоя на GitHub Pages при каждом push в `main`.
